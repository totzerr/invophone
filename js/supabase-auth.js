...
(function(){
 ...
 const importProductCatalogue=async(establishmentId,userId,products)=>{if(!client)return {error:'Service indisponible.'};const rows=(Array.isArray(products)?products:[]).map(p=>({establishment_id:establishmentId,name:String(p.n||p.name||'').trim(),category:String(p.cat||p.category||'Autres'),unit:String(p.u||p.unit||'unité'),low_stock_threshold:Number(p.seuil||0)||0,metadata:{legacy_id:p.id||null},created_by:userId})).filter(p=>p.name);if(!rows.length)return {error:'Aucun produit valide à importer.'};const result=await client.from('products').upsert(rows,{onConflict:'establishment_id,name'});return result.error?{error:errorMessage(result.error)}:{ok:true,count:rows.length};};
 ...
 window.SwaySupabaseAuth={available:function(){return !!client},identity:identity,signup:signup,signin:signin,reset:reset,updatePassword:updatePassword,finalizeWorkspace:finalizeWorkspace,workspaceMembers:workspaceMembers,loadWorkspaceState:loadWorkspaceState,saveWorkspaceState:saveWorkspaceState,importProductCatalogue:importProductCatalogue,watchWorkspaceState:watchWorkspaceState,stopWatchingWorkspaceState:stopWatchingWorkspaceState,isRecovery:function(){return recovery},isInviteFlow:function(){return invitationAccepted||String(location.hash||'').includes('type=invite')},signout:async function(){stopWatchingWorkspaceState();if(client)await client.auth.signOut()}};
})();
